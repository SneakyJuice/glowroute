    ...
    <div className="col-span-full">
      <div className="bg-white rounded-2xl border border-gold/35 shadow-featured hover:shadow-featured-hover hover:-translate-y-0.5 transition-all overflow-hidden grid grid-cols-1 md:grid-cols-[300px_1fr]">
        <div className="h-[220px] bg-gradient-to-br from-[#bdd4e7] to-[#d4ecf5] flex items-center justify-center text-5xl">
          {clinic.imageUrl ? <img src={clinic.imageUrl} alt={clinic.name} className="w-full h-full object-cover" /> : '✨'}
        </div>
        <div className="p-5 flex flex-col gap-2.5">
          ...
          <div className="flex items-center gap-2.5 mt-auto">
            ...
            {clinic.priceTier && <span className="ml-auto text-sm text-gray-500"><strong className="text-navy">{clinic.priceTier}</strong></span>}
          </div>
        </div>
      </div>
    </div>
...
export default function ClinicCard({ clinic }: ClinicCardProps) {
  const [saved, setSaved] = useState(false)
  return (
    <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm hover:shadow-lg hover:-translate-y-[3px] hover:border-teal/25 transition-all flex flex-col">
      <div className="h-[168px] relative overflow-hidden bg-gradient-to-br from-[#c9d8e8] to-[#e0eff7]">
        {clinic.imageUrl ? <img src={clinic.imageUrl} alt={clinic.name} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-4xl">🏥</div>}
      ...
      <div className="flex items-center gap-2 px-4 py-3 border-t border-gray-100">
        ...
        {clinic.priceTier && <span className="text-xs text-gray-500"><strong className="text-navy font-semibold">{clinic.priceTier}</strong></span>}
      </div>
    </div>
  )
}
