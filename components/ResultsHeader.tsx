...
export default function ResultsHeader({ count, city, view, sort, onViewChange, onSortChange }: Props) {
  return (
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-2.5">
      <div>
        <div className="text-[15px] text-gray-700"><strong className="font-bold text-onyx">{count} verified providers</strong> in {city}</div>
        <div className="text-xs text-gray-400 mt-0.5">Curated providers · sorted by relevance</div>
      </div>
      <div className="flex items-center gap-2.5">
        <div className="flex gap-0.5">
          {(['grid', 'list'] as const).map(v => (
            <button key={v} onClick={() => onViewChange(v)} className={`p-[7px] rounded border transition-all ${view === v ? 'bg-white border-gray-200 text-onyx shadow-sm' : 'border-transparent text-gray-400 hover:text-onyx'}`}>
