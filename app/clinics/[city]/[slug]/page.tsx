...
export async function generateMetadata({ params }: PageProps) {
  const clinic = flClinics.find(c => c.slug === params.slug)
  if (!clinic) return { title: 'Clinic Not Found | GlowRoute' }
  return {
    title: `${clinic.name} | GlowRoute — ${clinic.city}, FL`,
    description: clinic.description || `${clinic.name} is a verified medspa and aesthetic clinic in ${clinic.city}, FL. View treatments, reviews, and contact info on GlowRoute.`,
    openGraph: {
      title: clinic.name,
...
function StarRating({ rating }: { rating: number }) {
  return (
    <div className="flex gap-0.5">
      {[1,2,3,4,5].map(i => (
        <span key={i} className={`text-lg ${i <= Math.floor(rating) ? 'text-gold' : i - 0.5 <= rating ? 'text-gold opacity-50' : 'text-gray-200'}`}>★</span>
      ))}
    </div>
  )
}
...
export default function ClinicProfilePage({ params }: PageProps) {
  ...
  const googleMapsUrl = clinic.mapsUrl || `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(clinic.name + ' ' + (clinic.address || clinic.city + ' FL'))}`
  ...
  return (
    <div className="min-h-screen bg-[#F8F6F1] font-sans">
      ...
      <div className="w-full h-[260px] md:h-[340px] relative overflow-hidden bg-gradient-to-br from-[#0D1B2E] to-[#1a2e4a]">
        {primaryImage ? (
          <img src={primaryImage} alt={clinic.name} className="w-full h-full object-cover opacity-80" />
        ) : clinic.logo ? (
          <div className="w-full h-full flex items-center justify-center">
            <img src={clinic.logo} alt={clinic.name} className="max-h-[120px] max-w-[60%] object-contain opacity-60" />
          </div>
        ) : (
          <div className="w-full h-full flex items-center justify-center">
        ...
        <div className="absolute top-4 left-4 md:left-8">
          <Link href="/clinics" className="text-white/70 text-xs hover:text-white transition-colors flex items-center gap-1">
      ...
      <div className="max-w-5xl mx-auto px-4 -mt-16 relative z-10 pb-16">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-6">
          ...
          <div className="space-y-4">
              ...
              <div className="flex items-start gap-4">
                ...
                {clinic.logo && (
                  <div className="w-16 h-16 rounded-xl overflow-hidden border border-gray-100 flex-shrink-0 bg-gray-50 flex items-center justify-center">
                    <img src={clinic.logo} alt={`${clinic.name} logo`} className="w-full h-full object-contain p-1" />
                  </div>
                )}
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1.5">
                    ...
                    <span className="text-sm text-gray-400">({clinic.googleReviewCount} Google reviews)</span>
                  </div>
                  {clinic.address && (
                    <a href={googleMapsUrl} target="_blank" rel="noopener noreferrer"
                      className="flex items-center gap-1 mt-2 text-sm text-gray-500 hover:text-teal transition-colors">
                      <svg className="w-3.5 h-3.5 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20 10c0 6-8 12-8 12S4 16 4 10a8 8 0 0116 0z"/><circle cx="12" cy="10" r="3"/></svg>
                      {clinic.address}
                      <svg className="w-3 h-3 ml-0.5 opacity-50" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 13v6a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2h6M15 3h6v6M10 14L21 3"/></svg>
                    </a>
                  )}
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-2.5 mt-5">
                {clinic.phone && (
                  <a href={`tel:${clinic.phone}`}
                    ...
                    <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 16.92v3a2 2 0 01-2.18 2A19.79 19.79 0 0111.41 18 19.5 19.5 0 015 11.59a19.79 19.79 0 01-3.89-8.3A2 2 0 013.09 1h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L7.09 8.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16z"/></svg>
                    {clinic.phone}
                  </a>
                )}
                {clinic.website && (
                ...
                <a href={googleMapsUrl} target="_blank" rel="noopener noreferrer"
                  ...
                  <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20 10c0 6-8 12-8 12S4 16 4 10a8 8 0 0116 0z"/><circle cx="12" cy="10" r="3"/></svg>
                  Get Directions
                </a>
              </div>
            </div>
            ...
            {/* About */}
            {clinic.description && clinic.description.length > 80 && (
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                ...
                {clinic.brandValues && clinic.brandValues.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-4">
                    {clinic.brandValues.map(val => (
                      <span key={val} className="text-[11px] font-medium text-gray-500 bg-gray-50 border border-gray-200 px-2.5 py-1 rounded-full capitalize">
                        {val}
                      </span>
                    ))}
                  </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                  {galleryImages.map((img, i) => (
                    <div key={i} className="aspect-[4/3] rounded-lg overflow-hidden bg-gray-100">
                      <img src={img} alt={`${clinic.name} photo ${i + 2}`} className="w-full h-full object-cover hover:scale-105 transition-transform duration-300" />
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
              ...
              <div className="space-y-3 text-sm">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-medium text-navy">{clinic.city}, FL</span>
                </div>
                {clinic.priceTier && (
                  <div className="flex items-center justify-between">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="font-bold text-navy">{clinic.googleRating}</span>
                    <span className="text-gray-400 text-xs">({clinic.googleReviewCount})</span>
                  </div>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-gray-500">Status</span>
                  <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${clinic.verified ? 'bg-teal/10 text-teal' : 'bg-gray-100 text-gray-500'}`}>
            ...
            <div className="bg-navy rounded-2xl p-5 relative overflow-hidden">
              ...
              <h3 className="text-white font-bold text-sm mb-1.5 relative">Own this clinic?</h3>
              <p className="text-white/60 text-xs mb-4 relative leading-relaxed">Claim your listing to update info, add photos, and get featured placement.</p>
              <a href="/claim" className="block text-center bg-teal text-white text-sm font-semibold py-2.5 rounded-xl hover:bg-teal/80 transition-colors relative">
                Claim Free Listing →
              </a>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
              ...
              <div className="space-y-3">
                {flClinics
                  ...
                  .slice(0, 4)
                  .map(nearby => (
                    <Link key={nearby.slug} href={`/clinics/${nearby.city.toLowerCase().replace(/\s+/g, '-')}/${nearby.slug}`}
                      className="flex items-start gap-3 group">
                      <div className="w-10 h-10 rounded-lg overflow-hidden bg-gray-100 flex-shrink-0 flex items-center justify-center">
                        {nearby.logo
                          ? <img src={nearby.logo} alt={nearby.name} className="w-full h-full object-contain p-1" />
                          : <span className="text-lg">✨</span>
                        }
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-semibold text-navy group-hover:text-teal transition-colors line-clamp-1">{nearby.name}</p>
                        <div className="flex items-center gap-1 mt-0.5">
                          <span className="text-gold text-[10px]">★</span>
                          <span className="text-[10px] text-gray-500">{nearby.googleRating} ({nearby.googleReviewCount})</span>
                        </div>
                      </div>
                    </Link>
                  ))}
              </div>
              <Link href="/clinics" className="block text-center text-xs font-semibold text-teal mt-4 hover:underline">
